   ...
   function checkIPLimit(ip) {
       ...
       const now = Date.now();
       const oneHour = 30 * 60 * 1000; // 30 menit dalam milidetik
   ...
   router.get('/', async (req, res) => {
       try {
           ...
           const allDomains = response.data.result.filter(record => 
               record.type === 'A' && record.name.endsWith('clohostid.me')
           );
   ...
   router.post('/get-subdomain', async (req, res) => {
       ...
       if (!checkIPLimit(clientIP)) {
           ...
           return res.json({ 
               success: false, 
               message: 'Anda harus menunggu 30 Menit sebelum membuat subdomain baru' 
           });
       }
       ...
       try {
           ...
           if (result.success) {
               ...
               await UniqueCode.create({
                   code: uniqueCode,
                   subdomain: `${desiredSubdomain}.clohostid.me`,
                   serverIP,
                   proxyStatus
               });
